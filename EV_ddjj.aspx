<%@ Page Language="VB" AutoEventWireup="false" CodeFile="EV_ddjj.aspx.vb" Inherits="EV_ddjj" %>
<!DOCTYPE html>
                                    ...
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-btn">
                                            <button type="button" id="guardar" class="btn btn-block btn-default btn-lg" onclick="printDiv('formIMP')" value="Decargar Reporte">
                                                <span class="glyphicon glyphicon-floppy-disk"></span>Imprimir</button> 
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var idcontrato
               ...
               $.ajax({
                   type: 'POST',
                   async: false,// tipo de llamada (POST, GET)
                   url: 'EV_ddjj.aspx/impcontrato',     // el URL del método que vamos a llamar
                   data: '{"v1": "' + personadatos[0] +'"}',
                   contentType: "application/json; charset=utf-8",
                   dataType: "json",                           // tipo de datos enviados al servidor
                   success: function (laboral) {
                       if (laboral.d[0] == "error") {                            
                           alert("ERROR NO HAY DATOS COMUNIQUESE CON LA UNIDAD DE SISTEMAS");                     
                       } else {                            
                           ...
                           $('#contrato').text(contrato[laboral.d[3]-1]);
                           $('#categoria').text(categoria[laboral.d[2]-1]);
                           $('#ffinanciamiento').text(fuente[laboral.d[5]-1]);         
                           $('#horario').text(hora[laboral.d[6]-1]);
                           $('#item').text(laboral.d[4]);
                           
                           let date = new Date(laboral.d[7])
                           ...
                           if(month < 10){
                                ...
                                $('#finicio').text(`${day}/0${month}/${year}`);
                           }else{
                                //console.log(`${day}-${month}-${year}`)
                                $('#finicio').text(`${day}/${month}/${year}`);
                            }                          
                          

    //$('#finicio').text(laboral.d[7]);

   
    let date1 = new Date(laboral.d[8])
    if (!isNaN(new Date(date1))){
                                ...
                                $('#ffin').text(`${day1}/0${month1}/${year1}`);
                                }else{
                                    console.log(`${day1}-${month1}-${year1}`)
                                $('#ffin').text(`${day1}/${month1}/${year1}`);
    }
    }
                           //$('#ffin').text(laboral.d[8]);
                           idcontrato = laboral.d[0];                          
                       } 
                   },
 ...
 console.log(idcontrato)
               $.ajax({
                   ...
                   url: 'EV_ddjj.aspx/impcargo',     // el URL del método que vamos a llamar
                   data: '{"v1": "' + personadatos[0] + '", "v2": "' + idcontrato + '"}',
                   contentType: "application/json; charset=utf-8",
                   dataType: "json",                           // tipo de datos enviados al servidor
                   success: function (laboral) {
                       if (laboral.d[0] == "error") {
                           ...
                           $('#cargo').text(laboral.d[5]);
                           $('#servicio').text(laboral.d[11]);
                           $('#entidad').text(bloque[laboral.d[3]-1]); 
                           


                           const today = new Date(laboral.d[10]);
                           ...
                           const hora1 = today.getHours();
                           const minuto1 = today.getMinutes();

                           const fechatotal = formattedToday +' '+ hora1 +':'+minuto1;
                                
                           $('#fechaimp').text(fechatotal);
                           //$('#fechaimp').text(laboral.d[10]);
                       }
                   },
                   error: function (result) {          // función que se va a ejecutar si el pedido falla
                       alert("ERROR " + result.status + ' ' + result.statusText);
