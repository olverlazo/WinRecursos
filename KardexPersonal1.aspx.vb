...
Partial Class KardexPersonal1
    ...
    Public Shared Function CargarLista() As Object
        ...
        Using con As New SqlConnection(ConfigurationManager.ConnectionStrings("BDRRHHConnectionString").ConnectionString)
            Using cmd As New SqlCommand("select nombre + ' ' + ap_paterno + ' ' + ap_materno as nombres,  CAST(LTRIM(STR([documento] ,50)) AS NVARCHAR(50))+' '+ ISNULL(cast([complemento] as nvarchar(50)),'') +' '+ expedido as ci,CONVERT (char(10),f_nacimiento,103) as f_nacimiento,direccion,telefono,profesion  from dbo.personal order by nombres", con)
                con.Open()
                Dim da As New SqlDataAdapter(cmd)
                da.Fill(dt)
                If dt.Rows.Count > 0 Then
                    'For i As Integer = 0 To dt.Rows.Count - 2
                    For i As Integer = 0 To 300               'objPersona.Add(New persona() With
